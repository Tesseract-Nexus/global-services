...
type ValidateCredentialsRequest struct {
	...
	SkipPasswordValidation bool      `json:"skip_password_validation,omitempty"` // Skip password check, only check account status
}
...
func (s *TenantAuthService) ChangePassword(ctx context.Context, userID, tenantID uuid.UUID, currentPassword, newPassword string, changedBy *uuid.UUID, authContext string) error {
	...
	if err != nil {
		...
		return fmt.Errorf("SAME_PASSWORD: this password was recently used, please choose a different one")
	}
