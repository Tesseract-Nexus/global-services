...
export async function directAuthRoutes(fastify: FastifyInstance) {
  ...
  fastify.post<{
    ...
    if (!rateLimit.allowed) {
      logger.warn({ ip: maskIP(clientIP) }, 'Rate limit exceeded for tenant lookup');
  ...
  fastify.post<{
    ...
    const rateLimit = await sessionStore.checkRateLimit(rateLimitKey);
    if (!rateLimit.allowed) {
      logger.warn({ ip: maskIP(clientIP), email: maskEmail(email) }, 'Rate limit exceeded for login');
      return reply.code(429).send({
        success: false,
        error: 'RATE_LIMITED',
  ...
  fastify.post<{
    ...
    const rateLimit = await sessionStore.checkRateLimit(rateLimitKey);
    if (!rateLimit.allowed) {
      logger.warn({ ip: maskIP(clientIP), email: maskEmail(email) }, 'Rate limit exceeded for admin login');
      return reply.code(429).send({
        success: false,
        error: 'RATE_LIMITED',
        message: 'Too many login attempts. Please try again later.',
  ...
  fastify.post<{
    ...
    const rateLimit = await sessionStore.checkRateLimit(rateLimitKey);
    if (!rateLimit.allowed) {
      logger.warn({ ip: maskIP(clientIP) }, 'Rate limit exceeded for registration');
      return reply.code(429).send({
        success: false,
        error: 'RATE_LIMITED',
  ...
  fastify.post<{
    ...
    const rateLimit = await sessionStore.checkRateLimit(rateLimitKey);
    if (!rateLimit.allowed) {
      logger.warn({ ip: maskIP(clientIP), email: maskEmail(email) }, 'Rate limit exceeded for reactivation');
      return reply.code(429).send({
        success: false,
        error: 'RATE_LIMITED',
  ...
  fastify.post<{
    ...
    try {
      ...
      // Set session cookie using host-aware helper (sets bff_home_session for home hosts)
      const forwardedHost = request.headers['x-forwarded-host'] as string || request.hostname;
      setSessionCookie(reply, session.id, forwardedHost, remember_me);
